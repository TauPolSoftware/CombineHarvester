...
for WORKSPACE in `ls $1/{best_choice,m_vis_inclusive}/datacards/{individual/*/*,category/*,channel/*,combined}/workspace.root $1/workspace.root 2> /dev/null`
do
	pushd `dirname ${WORKSPACE}`
	
	# initial fit
	combineTool.py -M Impacts --doInitialFit --allPars \
		--redefineSignalPOIs pol --setParameters pol=-0.17528,r=1 --setParameterRanges pol=-0.3,0.0:r=0.5,1.5 \
		--robustFit 1 -t -1 \
